...
def calculate_allocation_by_age(age_years, max_age=85, min_equity=10):
    debt_pct = min((age_years / max_age) * 100, 100)
    equity_pct = 100 - debt_pct
    
    if equity_pct < min_equity:
        equity_pct = min_equity
        debt_pct = 100 - min_equity
...
with col1:
    ...
    if allocation:
        ...
        st.markdown(
            ...
            '         Retire confidently knowing your portfolio allocation balances growth (Equity) and safety (Debt) over time.'
            '</div>',
            unsafe_allow_html=True
        )
        st.markdown('</div>', unsafe_allow_html=True)
...
with col2:
    if allocation:
        ...
        # Allocation Boxes (MOVED TO TOP) - UNCHANGED
        st.markdown(
        ...
        #  GLIDE PATH SPARKLINE LOGIC
        
        st.markdown('<div style="height: 10px;"></div>', unsafe_allow_html=True)
        
        # 1. Generate Data (Ages 20 to 85)
        ages = np.arange(20, 86)
        equity_data = []
        debt_data = []
        
        for age in ages:
            equity, debt = calculate_allocation_by_age(age)
        ...
        # 2. Create Plotly Sparkline
        fig_spark = go.Figure()

        # Equity Line (Green)
        fig_spark.add_trace(go.Scatter(
            x=df_glide['Age'], y=df_glide['Equity'],
            mode='lines',
            name='Equity',
            line=dict(color='#0a6b32', width=5), # Deep Green for Equity
            fill='tozeroy',
            fillcolor='rgba(10, 107, 50, 0.2)'
        ))

        # Debt Line (Blue)
        fig_spark.add_trace(go.Scatter(
            x=df_glide['Age'], y=df_glide['Debt'],
            mode='lines',
            name='Debt',
            line=dict(color='#004e92', width=5), # Deep Blue for Debt
            fill='tonexty',
            fillcolor='rgba(0, 78, 146, 0.2)'
        ))
        
        # Current Age Marker (Vertical Line)
        current_age = allocation["age"]
        fig_spark.add_vline(x=current_age, line_width=2, line_dash="dash", line_color="orange")
        fig_spark.add_annotation(
            x=current_age, y=95, # Position near the top
            text=f"Your Age: {current_age}",
            showarrow=False,
            font=dict(color="orange", size=10),
            yanchor="top"
        )

        # 3. Configure Layout for Sparkline
        ...
        # Hide Axis Titles, Ticks, and Gridlines
        fig_spark.update_xaxes(
            title_text='', showgrid=False, zeroline=False, showticklabels=True,
            range=[20, 85] 
        )
        fig_spark.update_yaxes(
            title_text='', showgrid=True, zeroline=False, showticklabels=False,
            range=[0, 100] 
        )
